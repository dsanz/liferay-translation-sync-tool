...
function compute_locales() {
	for language_file in $(ls $target_dir/${FILE}${file_sep}*.$file_ext); do
		locale=$(echo $language_file | sed -r "s:$target_dir\/${FILE}${file_sep}([^\.]+).$file_ext:\1:")
		L[${#L[@]}]=$locale
	done
	locales="${L[@]}"
...
function set_lang_paths() {
	lang_file="${FILE}${file_sep}$1.${file_ext}";
