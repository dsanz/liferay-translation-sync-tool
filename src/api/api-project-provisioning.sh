...
function display_AP_projects() {
	...
	for git_root in "${!GIT_ROOTS[@]}"; do
		...
		loglc 6 $RED "$(printf "%-51s%s" "Project Code")$(printf "%-100s%s" "Build-lang path (relative to git_root)")$(printf "%-35s%s" "Lang file base (rel. to build lang)")$(printf "%-60s%s" "Project name")$(printf "%-5s%s" "Check") "
		while read project; do
			logt 3 -n "$(printf "%-51s%s" "$project")"
