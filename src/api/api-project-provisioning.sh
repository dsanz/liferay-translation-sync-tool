...
function display_AP_projects() {
	...
	for git_root in "${!GIT_ROOTS[@]}"; do
		...
		loglc 6 $RED "$(printf "%-55s%s" "Project Code")$(printf "%-50s%s" "Build-lang path (relative to git_root)")$(printf "%-45s%s" "Language files base (rel. to build lang)")$(printf "%-50s%s" "Project name")$(printf "%-5s%s" "Check") "
